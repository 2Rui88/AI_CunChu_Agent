...
const NavBar = ({ onChatToggle, chatOpen }) => {
  ...
  return (
    <StyledHeader>
      ...
      {user ? (
        <>
          {onChatToggle && (
            <StyledMenu mode="horizontal" style={{ minWidth: 60 }}
              selectedKeys={chatOpen ? ['chat'] : []}
              onClick={onChatToggle}
              items={[{ key: 'chat', icon: <RobotOutlined />, label: chatOpen ? '关闭助手' : 'AI助手' }]} />
          )}
          <UserMenu
          mode="horizontal"
          items={[
